package lib.unicrypt.src.main.java.ch.bfh.unicrypt.crypto.schemes.padding.classes;
...
public class ISOPaddingScheme
	...
	public static ISOPaddingScheme getInstance(ByteArrayMonoid byteArrayMonoid) {
		if (byteArrayMonoid == null || byteArrayMonoid.getBlockLength() >= ByteArray.BYTE_ORDER) {
			throw new IllegalArgumentException();
		}
		return new ISOPaddingScheme(byteArrayMonoid);
	}

	public static ISOPaddingScheme getInstance(int blockLength) {
		return ISOPaddingScheme.getInstance(ByteArrayMonoid.getInstance(blockLength));
	}
